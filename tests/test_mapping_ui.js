...
(async function () {
    ...
    assert(!template.includes('<details data-bind="visible: chamberLightPrinterBusy"'));
    assert(template.includes('value: draft, enable: $root.tuneLightingEnabled'));
    assert(template.includes('click: $root.applyPrintBrightness, enable: $root.tuneLightingEnabled'));
